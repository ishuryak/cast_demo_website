...
// 5. Nothing published under docs/ carries an internal tool-process note.
// Until 2026-09-25 every walkthrough served assets/art/manifest.json with a
// "note" field reading "Codex owns this manifest. Claude supplies assets and
// ART_NOTES.md; integration follows review." The page never displays that
// field, but GitHub Pages serves the file to anyone who requests it. A
// disclosure of AI assistance belongs in prose a reader is meant to see, not
// in a workflow note left inside a data file.
{
  const hits = [];
  const walk = (dir) => {
    for (const name of readdirSync(dir)) {
      ...
      if (!/\.(json|js|mjs|html|css|md|txt)$/.test(name)) continue;
      const t = readFileSync(f, "utf8");
      if (/\b(Codex|Claude|Gemini|Copilot)\s+(owns|supplies|adjudicates|integrates)\b/.test(t) ||
          /\bART_NOTES\.md\b/.test(t))
        hits.push(f.slice(ROOT.length + 1));
    }
  };
  walk(join(ROOT, "docs"));
  ok(`no published file carries an internal tool-process note` +
