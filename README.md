...
cast_demo_website/
  ...
  tests/
    ...
    test_clinical_contract.mjs  docs/clinical/: clinical units, the coverage
                             figures, acronym expansion, shared data file
  ...
  docs/                              <- the published static site (GitHub Pages root)
    ...
    clinical/                          clinical view: the same estimates in
                                       patients per 100, with interval coverage
...
6. **Nothing here measures coverage, and the intervals do not achieve it across
   ...
   both are 0 of 40. The clinical view of the site
   (`/clinical/`) recomputes these in the browser on every load and shows them
   on its face. A nominal 95% interval is not a measured 95% interval, and the
