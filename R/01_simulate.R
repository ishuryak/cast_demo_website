...
simulate_cohort <- function(n, conf_strength, shape, seed) {
  ...
  dat <- data.frame(age = age, stage = stage, ps = ps,
                    W = as.integer(W), Y = Y, D = D)
  ...
  list(data = dat,
       ...
       meta = list(shape = shape, conf_strength = conf_strength, n = n,
                   event_rate = mean(D),
                   treated_frac = mean(W),
                   # propensity-driven imbalance: SMD of age by arm (illustrative)
                   smd_age = (mean(age[W==1]) - mean(age[W==0])) /
                             sqrt((var(age[W==1]) + var(age[W==0])) / 2),
                   smd_ps = (mean(ps[W==1]) - mean(ps[W==0])) /
                            sqrt((var(ps[W==1]) + var(ps[W==0])) / 2)))
}
