...
`docs/oncology/`, `scripts/`, `tests/`, the CI workflow and `package.json` only.
No R script, no `scenarios.json`, no figure and no registry file is touched.
`docs/tutorial/` was rebuilt by `npm run build:pages` and is byte-identical.

The review, the four decisions Igor made before any file was touched, and the two
findings deliberately not acted on are in
[`development/2026_09_21_tutorial-variants.md`](development/2026_09_21_tutorial-variants.md).

### 30. The clinical walkthrough (v3) was not the walkthrough, and its chart could not load

- **What was wrong.** `/tutorial_v3/` was meant to be the illustrated walkthrough
  with a clinical framing. It was instead the methods page with the walkthrough's
  sections spliced in: it linked the methods page's `style.css`, so the
  walkthrough's markup rendered almost unstyled under a blue banner. It embedded
  about 500 lines of the methods page whose script was never loaded, so those
  controls, that figure and those cards were empty. Its own chart fetched a data file the
  previous commit had deleted, and the page scrolled sideways on every desktop.
- **The proof it was real.** At `b48bbb4`, `/tutorial_v3/data/scenarios.json`
  returned HTTP 404 live. The page had two `<main>` open tags and one close, three
  sections duplicated and 11 duplicated ids. `node tests/test_viewport_overflow.mjs
  docs/tutorial_v3` reported 12 failures, the widest element a 2078 px `IMG`.
- **The fix.** v3 is now generated by `scripts/build-tutorial-v3.mjs`: a copy of
  the built walkthrough plus 14 insertions, each anchored to a string that must
  occur exactly once or the build stops. It adds a clinician reading path, a hero
  note and five "In the clinic" asides (two active options and the number needed
  to treat, confounding by indication, reading the curve at a chosen horizon, the
  patient versus the cohort average, and what to ask of a registry), plus one
  stylesheet for the asides. `npm run build:pages` rebuilds it after the
  walkthrough.
- **The test that now pins it.** `tests/test_tutorial_variants.mjs`: every local
  link, asset and `fetch()` target resolves, no duplicated id, exactly one
  `<main>`, every in-page link has a target, and v3 keeps the walkthrough's
  stylesheets, scripts, data and chapter headings byte-for-byte. **Confirmed to
  fail against the unfixed tree: 9 failures at `b48bbb4`**, one per defect above.
  CI now also runs the overflow suite on v2 and v3, and its `git diff
  --exit-code` step covers `docs/tutorial_v3`, so a walkthrough change not carried
  into v3 fails CI.
- **The verification.** On 2026-09-21, node v18.19.1: `node
  tests/test_tutorial_variants.mjs` PASS (0 failures). `test_viewport_overflow.mjs`
  PASS on `docs`, `docs/tutorial`, `docs/tutorial_v2` and `docs/tutorial_v3`.
  `npm test` 12 of 12, as before. Rendered via `python3 -m http.server` at 1280 px:
  walkthrough styling, a loaded chart and the asides in place.

### 31. The oncology version (v2) linked to pages that do not exist

- **What was wrong.** v2's header linked to "methods and diagnostics",
  "clinical view" and "illustrated walkthrough" at `../../`, `../../clinical/` and
  `../../tutorial/`, one directory above the site. No clinical view exists.
- **The proof it was real.** From `/cast_demo_website/tutorial_v2/`, `../../`
  resolves to `ishuryak.github.io/`, outside the site. `test_tutorial_variants.mjs`
  reported the three as unresolved against the unfixed tree.
- **The fix.** Links now go to `../`, `../tutorial/` and `../tutorial_v3/`. The two
  source comments that described a `docs/clinical/` page were corrected. Two
  layout defects were fixed in the same pass: the unit under a large number now
  sits on its own line instead of wrapping beside it at the number's line height,
  and a seventh card left alone on the last row spans it.
- **The test that now pins it.** `tests/test_tutorial_variants.mjs`, as in entry 30.
- **The verification.** As in entry 30. The layout change was checked by render
  at 1280 px. The overflow suite passes on `docs/tutorial_v2` at all six widths.

### 32. A second live copy of v2 sat at /oncology/

- **What was wrong.** `docs/oncology/` was a byte-identical copy of v2, published
  at `/oncology/`, so any fix to one would silently leave the other behind.
- **The proof it was real.** `diff -r docs/oncology docs/tutorial_v2` was empty.
- **The fix.** Replaced with a redirect page (`meta refresh` and a canonical link)
  to `../tutorial_v2/`, so a link already shared keeps working.
- **The test that now pins it.** `tests/test_tutorial_variants.mjs` checks that
  the redirect target resolves.
- **The verification.** As in entry 30.
