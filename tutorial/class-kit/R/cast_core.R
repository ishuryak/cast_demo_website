...
# pipeline (time_varying_effects.R): Ledoit-Wolf covariance shrinkage, the
# cross-horizon influence-function covariance estimator, and the GLS quadratic
# trajectory fit. Kept self-contained so the demo runs the real method.
#
# (The covariance is built from the CSF doubly-robust influence-function scores.
# An earlier version used a fixed-forest patient bootstrap, whose band was only
# conditional on the forests; score_horizon_cov() below replaced it.)
...
# automatically when BOTH conditions hold -- Sigma is well-conditioned
# (cond <= gls_cond_max, default 100) AND the resulting quadratic actually fits
# (R^2 >= 0.5, the "fit is sane" guard, so a GLS solution that explains less than
# half the variation in the horizon estimates is not preferred to WLS). If either
# fails it falls back to weighted least squares (weights 1 / SE^2). Both
# thresholds are registered in constant_registry.yaml.
