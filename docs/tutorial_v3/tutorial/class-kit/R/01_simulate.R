...
UNMEAS_GRID <- if (SUB > 0) c(0) else c(0, 0.75, 1.5)   # latent-confounder strength (Gamma)
...
cat(sprintf("[01_simulate] N=%d  conf=%s  unmeas=%s  shapes=%s\n",
            N, paste(CONF_GRID, collapse=","), paste(UNMEAS_GRID, collapse=","),
...
simulate_cohort <- function(n, conf_strength, shape, seed, unmeas_strength = 0) {
  set.seed(seed)
  ...
  # --- LATENT (unmeasured) confounder: a fitness factor that both improves
  #     survival and raises the chance of treatment, and is NEVER given to any
  #     fitted model. It biases every estimate that cannot see it. ---
  u_hidden <- rnorm(n)                                # standardized latent fitness
  ...
  lp_surv <- -0.25 * z_age - 0.45 * z_stage + 0.30 * z_ps -
             ...
             0.45 * unmeas_strength * u_hidden        # latent, protective
  lambda0 <- exp(4.3 + lp_surv)                       # Weibull scale (months)
  # Marginal control-arm median survival (what a KM curve of the control arm
  # shows) is ~47 months. exp(4.3)*log(2)^(1/k) = 56.7 is the median at
  # lp_surv = 0 only; the cohort mean lp_surv is ~-0.14, mostly the smoking term.
  ...
  lp_treat <- conf_strength * (-0.5 * z_age - 0.6 * z_stage + 0.5 * z_ps -
                               0.45 * z_comorb) +
              0.60 * unmeas_strength * u_hidden        # latent drives assignment
  ...
  list(data = dat,
       ...
       meta = list(shape = shape, conf_strength = conf_strength,
                   unmeas_strength = unmeas_strength, n = n,
                   event_rate = mean(D),
                   treated_frac = mean(W),
                   smd_age = smd(age), smd_ps = smd(ps),
                   smd_comorb = smd(comorb), smd_smoke = smd(smoke),
                   smd_u_hidden = smd(u_hidden)))
}

# Everything below builds and writes the published scenario grid. It is skipped
# when this file is SOURCED for `simulate_cohort()` alone (the caller sets
# DEMO_SOURCE_ONLY first), so R/04_replicate_seeds.R can draw cohorts from the
# same generative model without rebuilding or overwriting output/sim.rds. Run as
# a script the flag does not exist and the block runs exactly as before.
...
for (shape in SHAPES) {
  for (cs in CONF_GRID) {
    for (um in UNMEAS_GRID) {
      sid <- sid + 1
      seed <- 1000 + sid
      sc <- simulate_cohort(N, cs, shape, seed, unmeas_strength = um)
      key <- sprintf("%s_conf%.2f_unmeas%.2f", shape, cs, um)
      scenarios[[key]] <- sc
      cat(sprintf("  [%s] events=%.2f  treated=%.2f  SMD(age)=%.2f  SMD(ps)=%.2f  SMD(latent)=%.2f  peakTrueATE=%.2f\n",
                  key, sc$meta$event_rate, sc$meta$treated_frac,
                  sc$meta$smd_age, sc$meta$smd_ps, sc$meta$smd_u_hidden,
                  max(sc$true_ate$true_ate)))
    }
  }
}

saveRDS(list(scenarios = scenarios, horizons = HORIZONS,
             conf_grid = CONF_GRID, unmeas_grid = UNMEAS_GRID,
             shapes = SHAPES, n = N),
        "output/sim.rds")
